...
import {
  ...
  Settings, LogOut, Menu, X, Users, MessageSquare,
  Image, Eye, ChevronRight
} from 'lucide-react'
        ...
        <div className="flex items-center justify-between px-4 h-16 border-b border-brand-slate/30 shrink-0">
          {open && (
            <Link href="/" target="_blank" className="flex flex-col">
              <span className="font-heading text-lg font-bold text-gold leading-none">Rofimain</span>
              <span className="text-[8px] tracking-[0.3em] text-brand-gray font-mono">CMS</span>
            </Link>
          )}
          <button
            onClick={() => setOpen(!open)}
            className="text-brand-silver hover:text-gold transition-colors ml-auto p-1"
          >
            ...
            <Link key={href} href={href} title={!open ? label : undefined}
              className={`flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg transition-all duration-200
                ${active(href)
                  ...
                  : 'text-brand-silver hover:bg-brand-slate/30 hover:text-brand-cream'}
            ...
            {pathname !== '/admin' && (
              <>
                ...
                <span className="text-brand-cream capitalize">
                  {navItems.find(n => n.href !== '/admin' && pathname.startsWith(n.href))?.label}
                </span>
              </>
            )}
          </div>
