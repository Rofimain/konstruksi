    ...
    <>
      <header
        className={cn(
          ...
          isScrolled
            ? 'bg-brand-dark/95 backdrop-blur-md border-b border-brand-slate/50 py-4'
            : 'bg-transparent py-6'
        )}
      >
        <div className="container-xl flex items-center justify-between">
          ...
          <Link href="/" className="flex flex-col group">
            <span className="font-heading text-2xl font-bold tracking-tight text-brand-cream group-hover:text-gold transition-colors duration-300">
              ROFIMAIN
            </span>
            <span className="text-[10px] tracking-[0.3em] text-gold uppercase font-mono -mt-1">
              Konstruksi
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-4">
            <a
              ...
              className="flex items-center gap-2 text-sm text-brand-silver hover:text-gold transition-colors duration-300"
            >
              <Phone size={14} />
          ...
          <button
            ...
            className="lg:hidden p-2 text-brand-cream hover:text-gold transition-colors"
      ...
      <div
        className={cn(
          'fixed inset-0 z-modal bg-brand-dark transition-all duration-500 lg:hidden',
      ...
      >
        <div className="flex flex-col h-full pt-24 pb-10 px-8 overflow-y-auto">
          {/* Mobile Logo */}
          <div className="mb-10">
            <div className="font-heading text-3xl font-bold text-gold">ROFIMAIN</div>
            <div className="text-xs tracking-[0.3em] text-brand-silver uppercase font-mono">Konstruksi</div>
          </div>
            ...
            {navLinks.map((link, i) => (
              <div key={link.label}>
                <Link
                  href={link.href}
                  className={cn(
                    'block py-4 text-2xl font-heading font-medium border-b border-brand-slate/30 transition-all duration-200',
                    'translate-x-0',
          ...
          <div className="mt-10 space-y-4">
            <a href={`tel:${siteConfig.phone}`} className="flex items-center gap-3 text-brand-silver">
